...
import com.dog.video.player.GSYBaseVideoPlayer;
import com.dog.video.player.StandardGSYVideoPlayer;
...
public class ListVideoUtil {
    ...
    private boolean hideKey = false;//隐藏按键
    private boolean needLockFull = false;//隐藏按键
    ...
    public void resolveFullBtn() {
        ...
        if (!isFull) {
            EventBus.getDefault().post(new LandscapeEvent());
            resolveToFull();
        } else {
    ...
    private void resolveToNormal() {
        ...
        //此处不能延迟。。要不会引发MainActivity底部切换栏变形的诡异问题。。。
        handler.postDelayed(new Runnable() {
            @Override
            public void run() {
                isFull = false;
